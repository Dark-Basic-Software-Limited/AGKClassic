...
int g_iNumSortedCommands = 0;
sCommand g_pSortedCommands[ MAX_COMMANDS ];
...
void ParseFile( sCommand* pCommands, int &numCommands, FILE *pFile )
{
	...
	do
	{
		...
		if ( SearchSortedArray( g_pSortedCommands, g_iNumSortedCommands, pCommands+numCommands ) >= 0 ) continue;
...
void SaveArray( sCommand *pCommands, int numCommands, FILE *pFile )
{
	...
	{
		// command name
		int length = strlen(pCommands[i].szCommandName);
		...
		length = strlen(pCommands[i].szReturnTypeText);
		...
		for ( int p = 0; p < pCommands[i].iNumParams; p++ )
		{
			...
			int length = strlen(pCommands[i].szParamsName[p]);
			...
			length = strlen(pCommands[i].szParamsTypeText[p]);
			fwrite( &length, 4, 1, pFile ); 
...
int main(int argc, char* argv[])
{
	...
	// $(BUILD_USER) is added to the preprocessor field in the project properties so it will define that value
		...
		FILE *pOldFile = fopen ( "../../../AGKTrunk/CommandParserNew/Final/CommandList.dat", "rb" );
	#endif
	
	if ( !pOldFile ) 
	{
		if ( MessageBox( NULL, "Couldn't find AGK Studio CommandList.dat, please copy it into CommandParserNew/Final", "Action Needed", MB_OKCANCEL ) != IDOK ) return 1;
		
		pOldFile = fopen( "CommandList.dat", "rb" );
		if ( !pOldFile ) 
		{
			MessageBox( NULL, "CommandList.dat not found", "Error", MB_OK );
			return 1;
		}
	}
	...
	if ( pOldFile )
	{
		if ( !LoadArray( g_pCommands, &g_iNumCommands, pOldFile ) ) return 1;

		for ( int i = 0; i < g_iNumCommands; i++ )
		{
			g_pSortedCommands[ i ] = g_pCommands[ i ];
		}

		// sort old commands, to check if they already exist when parsing files
		qsort( g_pSortedCommands, g_iNumCommands, sizeof(sCommand), CommandCompare );
		g_iNumSortedCommands = g_iNumCommands;
	...
	*/
	...
	for ( int i = 0; i < g_iNumCommands; i++ ) g_pSortedCommands[ i ] = g_pCommands[ i ];
	qsort( g_pSortedCommands, g_iNumCommands, sizeof(sCommand), CommandComparePlugin );
	g_iNumSortedCommands = g_iNumCommands;
	...
	OutputCommandPluginInterpreter( g_pSortedCommands, g_iNumSortedCommands, pFileOut );
	fclose( pFileOut );
	...
	// output IDE tags
	pFileOut = fopen( "../../IDE/Geany-1.24.1/data/tags/main.agc.tags", "w" );
	if ( !pFileOut )
	{
	...
	// if the command list was copied to local folder then delete it so we don't use an old version next time
	DeleteFile( "CommandList.dat" );
	/*
	...
	*/
	...
	FILE* pNewFile = fopen( "../../CompilerNew/CommandList.h", "wb" );
