...
void cFile::Init()
{
    ...
    currentpath = [g_pFileManager currentDirectoryPath];
	*g_szWriteDir = 0;
	if ( currentpath ) strcpy( g_szWriteDir, [currentpath UTF8String] );
	strcat( g_szWriteDir, "/" );
}
...
{
	unsigned long length = strlen( fromFile );
	...
	unsigned long lengthTo = strlen( toFile );
