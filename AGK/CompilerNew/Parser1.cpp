...
int Parser1::ParseLine( int includeFileID, const uString &sLine, const uString &sLineRaw, int iLineNum, uString &error, cProgramData *pProgram )
{
	...
	do
	{
		...
		if ( iCommentBlock ) token = ParseTokenCommentBlock( prevToken, str, consumed, error );
		else token = ParseToken( prevprevToken, prevToken, str, consumed, error );
		...
		else
		{
			...
			{
				char* szRaw = szBuffer;
				if ( consumed > 127 ) szRaw = new char[ consumed+1 ];

				if ( token == AGK_TOKEN_STRING )
				{
					int offset = (int)(str - sLine.GetStr());
				...
				{
					strncpy( szRaw, str, consumed );
					szRaw[ consumed ] = 0;
				}

				tokenID = pProgram->AddToken( token, szRaw, iLineNum, includeFileID );
				...
				if ( token == AGK_TOKEN_LABEL )
				{
					...
					{
						error.Format( "Inserted files cannot contain any labels or gosubs, use include files instead", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
						if ( consumed > 127 ) delete [] szRaw;
						return 0;
					}

					int index = pProgram->AddLabel( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), tokenID );
					if ( index < 0 ) 
					{
						...
						int prevIndex = pProgram->FindLabel( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
						int tokenID2 = pProgram->m_pLabels.m_pData[ prevIndex ]->iTokenID;
						const char* szInclude = g_pIncludeFiles.m_pData[ pProgram->m_pTokens[ tokenID2 ].iIncludeFile ]->sName.GetStr();
						int lineNum = pProgram->m_pTokens[ tokenID2 ].iLine;

						error.Format( "Label \"%s\" has already been defined on line %d in %s", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), lineNum, szInclude );
						if ( consumed > 127 ) delete [] szRaw;
						return 0;
					}
				}
				...
				if ( prevToken == AGK_TOKEN_INCLUDE && token == AGK_TOKEN_STRING )
				{
					...
					if ( includeFileID & 0x02000000 )
					{
						error.Format( "Inserted files cannot include other files", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
						if ( consumed > 127 ) delete [] szRaw;
						return 0;
					}

					int index = AddIncludeFile( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), 1, &error );
				...
				if ( prevToken == AGK_TOKEN_INSERT && token == AGK_TOKEN_STRING )
				{
					...
					if ( includeFileID & 0x02000000 )
					{
						error.Format( "Inserted files cannot insert other files", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
						if ( consumed > 127 ) delete [] szRaw;
						return 0;
					}

					int index = AddIncludeFile( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), 2, &error );
					if ( (index & 0xFFFFFF) == (includeFileID & 0xFFFFFF) ) 
					{
						error.SetStr( "Included file cannot include itself" );
						if ( consumed > 127 ) delete [] szRaw;
						return 0;
					}
					...
					Parser1 subParser;
					int result = subParser.ParseFile( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), index, pProgram );
					if ( result == 0 ) 
					{
				...
				if ( prevToken == AGK_TOKEN_DEC_FUNCTION )
				{
					...
					{
						if ( pProgram->FindCommandName( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() ) >= 0 )
						{
							error.Format( "Function \"%s\" is already a built in AppGameKit function", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
							return 0;
						}

						iFunctionLine = iLineNum;
						int index = pProgram->AddFunction( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), tokenID-1 );
						if ( index < 0 ) 
						{
							// do find after attempting to add as this case should be a rare event, and doing the find before the add would do two lookups
							int prevIndex = pProgram->FindFunction( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
							int tokenID2 = pProgram->m_pFunctions.m_pData[ prevIndex ]->iTokenID;
							const char* szInclude = g_pIncludeFiles.m_pData[ pProgram->m_pTokens[ tokenID2 ].iIncludeFile ]->sName.GetStr();
							int lineNum = pProgram->m_pTokens[ tokenID2 ].iLine;

							error.Format( "Function \"%s\" has already been defined on line %d in %s", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), lineNum, szInclude );
				...
				if ( prevToken == AGK_TOKEN_DEC_TYPE )
				{
					if ( token == AGK_TOKEN_IDENTIFIER )
					{
						...
						iTypeLine = iLineNum;
						int index = pProgram->AddType( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), tokenID-1 );
						if ( index < 0 ) 
						{
							// do find after attempting to add as this case should be a rare event, and doing the find before the add would do two lookups
							int prevIndex = pProgram->FindType( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
							int tokenID2 = pProgram->m_pTypes.m_pData[ prevIndex ]->iTokenID;
							const char* szInclude = g_pIncludeFiles.m_pData[ pProgram->m_pTokens[ tokenID2 ].iIncludeFile ]->sName.GetStr();
							int lineNum = pProgram->m_pTokens[ tokenID2 ].iLine;

							error.Format( "Type \"%s\" has already been defined on line %d in %s", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), lineNum, szInclude );
				...
				{
					int index = pProgram->AddConstant( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), tokenID-1 );
					if ( index < 0 ) 
					{
						// do find after attempting to add as this case should be a rare event, and doing the find before the add would do two lookups
						int prevIndex = pProgram->FindConstant( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
						int tokenID2 = pProgram->m_pConstants.m_pData[ prevIndex ]->tokenID;
						const char* szInclude = g_pIncludeFiles.m_pData[ pProgram->m_pTokens[ tokenID2 ].iIncludeFile ]->sName.GetStr();
						int lineNum = pProgram->m_pTokens[ tokenID2 ].iLine;

						error.Format( "Constant \"%s\" has already been defined on line %d in %s", pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr(), lineNum, szInclude );
				...
				{
					pProgram->m_sCompanyName.SetStr( pProgram->m_pTokens[ tokenID ].sValueRaw.GetStr() );
				}
...
int Parser1::ParseFile( const char* filename, int includeFileID, cProgramData *pProgram )
{
	...
	pProgram->AddToken( AGK_TOKEN_EOL, "\n", iLineCount, includeFileID );

	iLineCountTotal += iLineCount;
	return iLineCount;
}
