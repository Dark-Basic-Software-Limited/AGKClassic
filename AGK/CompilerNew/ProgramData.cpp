...
{
    const char* szInclude = g_pIncludeFiles.m_pData[ m_pTokens[ tokenID ].iIncludeFile ]->sName.GetStr();
	int lineNum = m_pTokens[ tokenID ].iLine;
...
int cProgramData::AddToken( AGKToken type, const char* raw, int line, int includeFileID )
{
	if ( m_iNumTokens+1 > m_iSizeTokens )
	{
		stToken *pNewArray = new stToken[ m_iSizeTokens+50000 ];
		for ( unsigned int i = 0; i < m_iNumTokens; i++ )
		{
			pNewArray[ i ] = m_pTokens[ i ];
		}
	...
	m_pTokens[ m_iNumTokens ].iIncludeFile = includeFileID & 0xFFFFFF;
	m_pTokens[ m_iNumTokens ].iLine = line;
	m_pTokens[ m_iNumTokens ].iType = type;
	if ( type == AGK_TOKEN_INTEGER ) 
	{
		if ( *raw == '%' ) m_pTokens[ m_iNumTokens ].iValueInt = (int) strtoul( raw+1, 0, 2 ); // binary number
		else if ( *raw == '0' && *(raw+1) == 'x' ) m_pTokens[ m_iNumTokens ].iValueInt = (int) strtoul( raw+2, 0, 16 ); // hex number
		else m_pTokens[ m_iNumTokens ].iValueInt = atoi( raw ); // base 10
	}
	if ( type == AGK_TOKEN_FLOAT ) m_pTokens[ m_iNumTokens ].fValueFloat = (float) atof( raw );
	if ( type == AGK_TOKEN_STRING ) 
	{
		m_pTokens[ m_iNumTokens ].sValueRaw.SetStr( raw+1 );

		if ( *raw == '\'' )
		{
			m_pTokens[ m_iNumTokens ].sValueRaw.ReplaceStr( "\\'", "'" );
			m_pTokens[ m_iNumTokens ].sValueRaw.ReplaceStr( "\\n", "\n" );
			m_pTokens[ m_iNumTokens ].sValueRaw.ReplaceStr( "\\r", "\r" );
			m_pTokens[ m_iNumTokens ].sValueRaw.ReplaceStr( "\\t", "\t" );
			m_pTokens[ m_iNumTokens ].sValueRaw.ReplaceStr( "\\\\", "\\" );
		}

		int index = AddString( m_pTokens[ m_iNumTokens ].sValueRaw.GetStr() );
		if ( index < 0 ) 
		{
			TokenError( "Failed to add string to string table", m_iNumTokens );
			return m_iNumTokens-1;
		}
		m_pTokens[ m_iNumTokens ].iStringIndex = index;
	}
	else if ( type == AGK_TOKEN_EOL ) 
	{
		m_pTokens[ m_iNumTokens ].sValueRaw.SetStr( "End Of Line" );
	}
	else m_pTokens[ m_iNumTokens ].sValueRaw.SetStr( raw );

	// remove colon at the end of the label
	if ( type == AGK_TOKEN_LABEL ) m_pTokens[ m_iNumTokens ].sValueRaw.Trunc2(1);
...
int cProgramData::GetFunctionLineNum( const char *name )
{
	...
	return m_pTokens[ tokenID ].iLine;
}
...
int cProgramData::AddPlugin( const char* name, const char* asname, uString *error )
{
	uString sName( name );
	sName.Strip( "\\/" );
	...
	if ( !found )
	{
		...
		while ( pluginFile.ReadLineFast( sLine ) )
		{
			...
			// read command name
			sLine.GetToken2( ',', 1, newPlugin.m_pCommands[ commandCount ].sTier1Name );
			...
			{
				if ( error ) error->Format( "Plugin command %s has an invalid return type %s", newPlugin.m_pCommands[ commandCount ].sTier1Name.GetStr(), temp.GetStr() );
				return 0;
			}
			...
			{
				if ( error ) error->Format( "Plugin command %s has an invalid parameter type, use 0 if there are no parameters", newPlugin.m_pCommands[ commandCount ].sTier1Name.GetStr() );
				return 0;
			}
			...
			{
				if ( error ) error->Format( "Plugin command %s has too many parameters, max is 32", newPlugin.m_pCommands[ commandCount ].sTier1Name.GetStr() );
				return 0;
			}
			...
			for ( int i = 0; i < newPlugin.m_pCommands[ commandCount ].iNumParams; i++ )
			{
				...
				{
					if ( error ) error->Format( "Plugin command %s has an invalid parameter type %c", newPlugin.m_pCommands[ commandCount ].sTier1Name.GetStr(), param );
					return 0;
				}
			}
	...
	if ( !found )
	{
		...
		if ( index < 0 )
		{
			if ( error ) error->Format( "Plugin reference %s is already being used by another plugin", sRefName.GetStr() );
			return 0;
		}
	}
...
{
	stCommand* c1 = (stCommand*) a;
	stCommand* c2 = (stCommand*) b;
...
/*
bool cProgramData::LoadCommandsArray( const char* filename, uString &error )
{
	if ( !filename ) 
	{
		error.SetStr( "Invalid command list filename" );
		return false;
	}

	cFile oCommandList;
	if ( !oCommandList.OpenToRead( filename ) )
	{
		error.SetStr( "Could not find CommandList.dat file" );
		return false;
	}

	m_iNumCommands = oCommandList.ReadInteger();
	...
	for( int i = 0; i < m_iNumCommands; i++ )
	{
		...
		int length = oCommandList.ReadInteger(); // name length
		if ( length >= 199 )
		{
			error.SetStr( "Failed to load commands array, command name too long" );
			return false;
		}
		oCommandList.ReadData( m_pCommands[i].szCommandName, length ); // name
		m_pCommands[i].szCommandName[ length ] = 0;
		temp.SetStr( m_pCommands[i].szCommandName );
		temp.Lower();
		strcpy( m_pCommands[i].szCommandName, temp.GetStr() );
		m_pCommands[i].iReturnType = oCommandList.ReadByte();
		m_pCommands[i].iNumParams = oCommandList.ReadByte();
		...
		{
			m_pCommands[i].iParamsTypes[p] = oCommandList.ReadByte();
		}
	}
...
bool cProgramData::LoadCommandsArrayLocal( uString &error )
{
...
int cProgramData::FinalCheck()
{
	...
	for ( int i = 0; i < m_pVariables.m_iCount; i++ )
	{
		if ( m_pVariables.m_pData[ i ]->iInitialised == 0 )
		{
			...
			uString error;
			error.Format( "Variable \"%s\" is used without being defined or initialised", m_pVariables.m_pData[ i ]->sName.GetStr() );
	...
	for ( int i = 0; i < m_pFunctions.m_iCount; i++ )
	{
		...
		for ( int v = 0; v < pFunction->pFuncVars.m_iCount; v++ )
		{
			if ( pFunction->pFuncVars.m_pData[ v ]->iInitialised == 0 )
			{
				...
				uString error;
				error.Format( "Variable \"%s\" is used without being defined or initialised", pFunction->pFuncVars.m_pData[ v ]->sName.GetStr() );
				TokenError( error, tokenID );
				//return 0;
				has_error = 1;
			}
...
int cProgramData::BuildBytecodeFile( const char *filename, int debugMode )
{
	...
	oBytecode.WriteInteger( company_length + 4 ); // header size
	...
	oBytecode.WriteInteger( g_pIncludeFiles.m_iCount );
	for ( int i = 0; i < g_pIncludeFiles.m_iCount; i++ )
	{
		int length = g_pIncludeFiles.m_pData[i]->sName.GetLength();
		oBytecode.WriteInteger( length );
		oBytecode.WriteData( g_pIncludeFiles.m_pData[i]->sName.GetStr(), length );
	}
